...
class reservation extends common {
    ...
    function printroom() {
        ...
        $sql = "SELECT id_reservation, GROUP_CONCAT(no) AS nos, SUM(amount) AS total  FROM {$this->prefix}mr WHERE id_reservation='{$id}'";
        $data = $this->m->fetch_assoc($sql);
        $this->sm->assign("mr", $data);
...
//$this->pr($data);exit;
    ...
    function showguest() {
        ...
        $sql = "SELECT no, roomnumber, person, daysstay, name, mobile, address, discount, gstamt, total FROM {$this->prefix}reservation 
                WHERE FIND_IN_SET({$room}, roomnumber) AND !cancel_by AND 
                    (('$date' BETWEEN date AND depature_date) OR (date >= '{$date}' AND depature_date IS NULL))";
        $data = $this->m->fetch_assoc($sql);
        echo json_encode($data);
        exit;
    }

...
class report extends common {
    ...
    function inout() {
        ...
        $time = $this->ini['in-out']['time'];
        $grace = $this->ini['in-out']['grace'];
        $sql = "SELECT *, 'Room' AS rtype FROM {$this->prefix}reservation WHERE (depature_date >= '$sdate' AND depature_date <= '$edate') AND cancel_date IS NULL ORDER BY depature_date, billno";
        $data = $this->m->sql_getall($sql);
        foreach ($data as $k => $v) {
            $stay = $v['daysstay'] ? $v['daysstay'] : 1;
            ...
            $days = $days_diff->d;
            // $this->pr($v['date']."---".$v['depature_date']."<br>");
            // $this->pr($v['no']."---".$days."<br>");
            // $this->pr($days_diff);
            $to_time = strtotime($v['time']);
            $from_time = strtotime($v['depature_time']);
            $timediff =  ($from_time - $to_time) / 60;
            if ($timediff > $grace*60) {
                // $this->pr($v['time']);
                // $this->pr($v['depature_time']);
                // $this->pr($from_time);
                // $this->pr($to_time);
                // $this->pr($timediff);
                ++$days;

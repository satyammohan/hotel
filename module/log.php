...
class log extends common {
    ...
    function checklog() {
        ...
        $this->m->query($sql);
        $sql = "CREATE TABLE IF NOT EXISTS {$this->prefix}log (id_log INT, type VARCHAR(1), date date DEFAULT NULL, `previous` text DEFAULT NULL, 
                `current` text DEFAULT NULL, `change` text DEFAULT NULL, PRIMARY KEY (id_log));";
        $this->m->query($sql);
        $sql = "SET sql_notes = 1;";
        $this->m->query($sql);
    }
    ...
    function voucher() {
        ...
        $sql = "SELECT * FROM {$this->prefix}log WHERE (date >= '$sdate' AND date <= '$edate') AND type='V' ORDER BY date, id_log";
        $data = $this->m->sql_getall($sql);
        $this->sm->assign("data", $data);
    }
}

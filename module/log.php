...
class log extends common {
    ...
    function create_log() {
        $sql = "SET sql_notes = 0;";
        $this->m->query($sql);
        $sql = "CREATE TABLE IF NOT EXISTS log (id_log INT NOT NULL AUTO_INCREMENT, type VARCHAR(1), date DATETIME DEFAULT NULL, 
        ...
        $this->m->query($sql);
        $sql = "ALTER TABLE reservation ADD `approve` TINYINT(1) NULL DEFAULT '0'";
        $this->m->query($sql);
        $sql = "ALTER TABLE mr ADD `approve` TINYINT(1) NULL DEFAULT '0'";
        $this->m->query($sql);        
    ...
    function booking() {
        ...
        $sql = "SELECT * FROM log WHERE (date(date) >= '$sdate' AND date(date) <= '$edate') AND type='B' ORDER BY date, id_log";
    ...
    function voucher() {
        ...
        $sql = "SELECT * FROM log WHERE (date(date) >= '$sdate' AND date(date) <= '$edate') AND type='V' ORDER BY date, id_log";
        $data = $this->m->sql_getall($sql);
        $this->sm->assign("log", $data);
    }
    function mr() {

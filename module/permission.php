...
class permission extends common {
    ...
    function module() {
        ...
        $module = $this->m->getall($this->m->query($sql));
        $this->sm->assign("module", $module);
    }   
    function permission() {
        $sql = "SELECT * FROM `permission`";
        $permission = $this->m->getall($this->m->query($sql));
        if (count($permission)==0) {
            $u = $_SESSION['id_user'];
            $sql = "INSERT INTO permission (name,status,ip,id_create,id_modify) VALUES ('INSERT',0,'','$u','$u')";
            $this->m->query($sql);
            $sql = "INSERT INTO permission (name,status,ip,id_create,id_modify) VALUES ('UPDATE',0,'','$u','$u')";
            $this->m->query($sql);
            $sql = "INSERT INTO permission (name,status,ip,id_create,id_modify) VALUES ('DELETE',0,'','$u','$u')";
            $this->m->query($sql);
            $sql = "INSERT INTO permission (name,status,ip,id_create,id_modify) VALUES ('REPORT',0,'','$u','$u')";
            $this->m->query($sql);
        }
        $this->sm->assign("permission", $permission);
    }
    function saveconfig() {
        $id = $_REQUEST['id'];
        $cid = $_REQUEST['cid'];
    ...
    function save() {
        ...
        if (isset($_REQUEST['set']) && $_REQUEST['set']) {
            $v = $_REQUEST['set'];
            $su = $_SESSION['id_user'];
            $sql = "INSERT INTO module_map (id_user, id_module, id_permission, permission, status, ip, id_create, id_modify) 
                    VALUES ($u, $m, $p, $v, 0, '', '$su', '$su') ";
            $this->m->query($sql);
        }
        echo "Successfully Updated.";
        exit;
    ...
    function saveall() {
        ...
        if (isset($_REQUEST['set']) && $_REQUEST['set']) {
            ...
            $su = $_SESSION['id_user'];
            $sql = "INSERT INTO module_map (id_user, id_module, id_permission, permission, status, ip, id_create, id_modify) 
                    SELECT '$u', '$m', id_permission, '$v', 0, '', '$su', '$su' FROM permission";
            $this->m->query($sql);
        }
        echo "Successfully Updated.";
        exit;

...
class banquet extends common {
    ...
    function insert() {
        ...
        $sql = $this->create_insert("banquet", $data);
        $this->m->query($sql);
    ...
    function update() {
        ...
        $sql = $this->create_update("banquet", $data, "id_banquet='{$_REQUEST['id']}'");
        $this->m->query($sql);
    ...
    function edit() {
        ...
        if ($id) {
            $sql = $this->create_select("banquet", "id_banquet='{$id}'");
            $data = $this->m->fetch_assoc($sql);
        } else {
            $sql = "SELECT max(no) AS no, max(bill) AS bill FROM banquet WHERE cancel_by=0";
            $data = $this->m->fetch_assoc($sql);
            $data['no'] = $data['no'] + 1;
    ...
    function listing() {
        $sql = "SELECT b.*, SUM(m.amount) AS received FROM banquet b LEFT JOIN mr m
    ...
    function cancel() {
        if (isset($_REQUEST['id'])) {
            ...
            $sql = $this->create_update("banquet", $data, "id_banquet='$id'");
            $this->m->query($sql);
    ...
    function cancellist() {
        $sql = "SELECT r.*, u.name AS uname FROM banquet r, user u
    ...
    function printbanquet() {
        ...
        $sql = "SELECT id_reservation, GROUP_CONCAT(no) AS nos, SUM(amount) AS total  FROM mr WHERE id_reservation='{$id}' AND cancel_date IS NULL AND mrtype='B'";
        $data = $this->m->fetch_assoc($sql);
        $this->sm->assign("mr", $data);

        $sql = "SELECT r.*, u.name AS username FROM banquet r LEFT JOIN user u ON r.id_create=u.id_user  WHERE r.id_banquet='{$id}'";
        $data = $this->m->fetch_assoc($sql);
    ...
    function printmr() {
        ...
        $sql = "SELECT r.roomnumber, r.name, r.address, m.*, u.name AS username 
            FROM mr m LEFT JOIN user u ON m.id_user=u.id_user, banquet r 
    ...
    function savemr() {
        ...
        $data['ip'] = $_SERVER['REMOTE_ADDR'];
        $data['mrtype'] = "B";
        $sql = $this->create_insert("mr", $data);
        $this->m->query($sql);

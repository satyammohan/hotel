...
class rooms extends common {
    ...
    function insert() {
        $data = $_REQUEST['room'];
        $this->m->query($this->create_insert("rooms", $data));
        $_SESSION['msg'] = "Record Successfully Inserted";
        $this->redirect("index.php?module=rooms&func=listing");
    }
    function update() {
        $data = $_REQUEST['room'];
        $sql = $this->create_update("rooms", $data, "id_rooms='{$_REQUEST['id']}'");
        $this->m->query($sql);
    ...
    function edit() {
        ...
        $sql = $this->create_select("rooms", "id_rooms='{$id}'");
    ...
    function listing() {
        $sql = "SELECT r.*, rt.name AS rtname FROM rooms r, roomtype rt WHERE rt.id_roomtype=r.id_roomtype ORDER BY rt.name, r.name";
